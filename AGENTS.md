...
#### Coze 文字立墙
世界中心沙质平地上用品牌粉色方块搭建 "Coze" 立墙（C 大写，其余小写）：
- **方块色**: #F46B95 粉色（COZE_CYAN 类型），带微噪纹理
- **字母规格**: 9×9 像素位图，字母间隔 2 格，总宽度 42 格
- **立墙结构**: XY 平面立墙，3 格厚（Z=-1~1），9 格高（Y=15~23），立于沙质地面（Y=14）
- **平地范围**: X 方向 ±22、Z 方向 ±10 的矩形区域，地面统一铺平至 y=18（沙层+石基）
- **核心方法**: `LETTERS` 静态对象（C/o/z/e 位图）、`_getTextBlock()`（XY 平面映射）、`_isInTextZone()`、`generateChunkData()` 中文字区分支、`_generateDecorations()`（装饰物生成）
- **玩家出生点**: (0, 地表+3, 22)，面朝正北观看立墙（pitch=-0.35 微俯视）

#### 空地装饰系统
Coze 立墙周围空地的环境点缀，使用确定性伪随机（基于坐标 hash）保证装饰位置一致：
